...
│                                                                               │
│  ┌─────────┐  F/D  ┌─────────┐ ┌─────────┐  D/E  ┌─────────┐  E/M  ┌─────────┐  M/W  ┌─────────┐│
│  │  FETCH  │──────▶│ DECODE  │─▶│ ISSUE   │──────▶│EXECUTE  │──────▶│ MEMORY  │──────▶│WRITEBK ││
│  │(fetch_  │  reg  │(decode_ │ │(issue_ │  reg  │(execute_│  reg  │(memory_ │  reg  │(write-  ││
│  │ cycle)  │       │ cycle)  │ │ stage)  │       │ cycle)  │       │ cycle)  │       │back_cyc)││
│  └────┬────┘       └────┬────┘ └────┬────┘       └────┬────┘       └────┬────┘       └────┬────┘│
│       │                 │           │                 │◄─(muldiv)       │                 │     │
│       └─────────────────┴───────────┴─────────────────┴─────────────────┴─────────────────┘     │
│                                          │                                            │
...
```
 ...
 Cycle N+1: ADD   LW    BUBBLE   (prev)  (prev)   ← stall + flush (bubble)
                  ▲ stall        ↑ flush_pipeline_2_3=1 (do ~issue_valid)
                  │ stall_f,d=1 (do issue_stall=1)
